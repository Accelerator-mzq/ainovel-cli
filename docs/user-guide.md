...
| **共创规划** | 想法模糊 | 与 AI 多轮对话澄清需求，右侧实时同步整理出的创作指令草稿；AI 每轮给 1-3 条建议，按数字键一键填入；满意后 `Ctrl+S` 或直接对 AI 说「开始」（弹出确认框后 Enter 开写）进入正式创作 |

两种方式最终收敛为同一份创作指令，进入同一套引擎。

### 2.1.1 导入已有设定

如果你已有世界观文档、人物设定、境界体系等资料，可以在启动前放到 `./settings/` 目录，引擎启动时自动全文读取并注入 Architect 的规划上下文，无需粘贴、无需输入框操作。
...
- 支持 `.md` / `.txt` / `.markdown` 文件，递归扫描子目录，按路径字典序排列
- 单文件上限 3 万字（rune），全部文件合计上限 6 万字（超限自动截断并提示）
- 目录不存在或为空时行为零变化（不报错、不清空已落盘的设定）
- 启动时 TUI 事件栏显示"已加载用户设定：N 个文件，共 M 字"
- 设定落盘为 `output/{书名}/user_settings.md`，由 Architect 规划时全程遵循

**与 rules.md 的分工：**

| 文件 | 用途 |
|---|---|
| `settings/*.md` | 世界观/人物全文素材（最高优先级规划依据，AI 补全缺失细节） |
| `rules.md` | 持续性硬约束与红线（字数/禁词/风格规则，每章机械检查） |

> 生效时机：设定主要在**开书规划期**与后续**弧/卷展开**时被 Architect 读取遵循；已生成的设定档案（foundation）不会因修改 `settings/` 后重启而被追溯重写，要修改已写内容请用干预指令。
...
2. **大纲审阅**（TUI 默认开启）：规划完成后系统自动暂停，提示你查看
   `output/{书名}/layered_outline.md`。输入修改意见会调整大纲后再次暂停，
   输入「开始」（或「确认」「开写」「开始写作」）进入正式写作。
   headless 默认跳过此环节；配置 `plan_review: "on"/"off"` 可覆盖两端。
3. **写作期**：Writer 逐章完成 构思 → 写正文 → 一致性自查 → 提交。每章提交后系统自动维护摘要、时间线、伏笔台账、角色状态。
4. **弧末/卷末**：Editor 做七维评审（设定一致性/角色行为/节奏/叙事连贯/伏笔/钩子/审美），评分不过线的章节进入重写或打磨队列；随后生成弧/卷摘要，Architect 展开下一弧或创建下一卷。
5. **完结**：规划的章节全部写完、活跃伏笔归零、指南针长线收束后自动收尾。
...
- 落盘到 `simulate/personas/<作者名>/`，每条 URL 一个 `.txt` 文件；同一 URL 重抓会覆盖旧文件。作者名将用作目录名，不能含空格或 `<>:"/\|?*` 等字符。
...
| `plan_review` | string | 否 | 规划后暂停审阅大纲：`"auto"`（TUI 开/headless 关，默认）/`"on"`/`"off"` |
...
├── user_settings.md         # 用户外部设定文档（settings/ 目录内容 + 共创原文，Architect 规划依据）
└── meta/
...
**Q: 怎么把已有世界观/人物设定文档导入引擎？**
推荐方式：把 `.md` / `.txt` 文件放到 `./settings/` 目录，启动时自动全文读入并注入 Architect 规划上下文（最高优先级，绕过输入框长度限制和共创草稿有损压缩）。详见 [2.1.1 导入已有设定](#211-导入已有设定)。

**Q: 共创里贴的大段设定会丢吗？**
不会丢。`Ctrl+S` 进入创作时，对话中全部用户原文会自动保全并追加到 `output/{书名}/user_settings.md`（"共创对话用户原文（备查）"段），Architect 规划时会参考。但推荐长设定直接走 `settings/` 目录，避免对话裁剪带来的不确定性。
